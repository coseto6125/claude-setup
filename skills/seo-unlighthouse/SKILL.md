...
All flags forward through `"$HOME/.claude/skills/seo/bin/claude-seo" run unlighthouse_run.py`, which handles
...
- For LCP subpart decomposition on slow pages, use the
  `"$HOME/.claude/skills/seo/bin/claude-seo" run lcp_subparts.py` workflow (Phase C).
