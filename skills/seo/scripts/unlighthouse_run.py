...
def run(
    ...
    *,
    ...
    # Resolve before the run: the subprocess starts in a temp cwd (see below), so a
    # relative --output-path would land somewhere the caller never looks.
    out_dir = Path(output_dir).resolve() if output_dir else Path(tempfile.mkdtemp(
    ...
    # Run from a temp dir, not the caller's cwd. Unlighthouse drives Lighthouse, whose
    # chrome-launcher sees WSL and takes its Windows branch, so the throwaway Chrome
    # profile is built at a `C:\Users\...\lighthouse.<pid>` path. That is not a real
    # path on Linux, so mkdirSync creates ONE directory whose literal name contains
    # backslashes, relative to cwd. Audit a site from a repo root and every worker
    # leaves one of those folders behind. Clearing TMP / TEMP / LOCALAPPDATA does not
    # help: the fallback path is Windows-shaped too. Moving cwd puts the junk somewhere
    # harmless whichever branch it takes.
    try:
        proc = subprocess.run(
            cmd,
