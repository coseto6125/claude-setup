...
Follow both.
target = <url>
output_dir = <audit dir>
```

There is no per-dimension agent definition. `specs/analyst.md` carries the mechanics every
dimension shares; `specs/<dimension>.md` carries that dimension's steps, thresholds, findings
filename, and audit-data category.
