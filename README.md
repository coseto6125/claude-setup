...
**Hooks execute on every matching event.** `hooks/` holds five scripts and `settings.example.json` wires four of them: `auto-etoon.sh` and `limit-worktrees.sh` on `PreToolUse`, `idle-guard-stop.sh` on `Stop`, `idle-guard-submit.sh` on `UserPromptSubmit`. `worktree-symlinks.sh` ships unwired, so wire it yourself or delete it. Read each one before you install it.
